...
import { prepareWebxpayRedirectAction } from "@lib/data/webxpay"
...
export default function WebxpayRedirectClient({ countryCode, cartId }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (process.env.NODE_ENV !== "production") {
        console.info("[webxpay] prepare-redirect starting", { cart_id: cartId })
      }

      const prepared = await prepareWebxpayRedirectAction()
      if (!prepared.ok) {
        if (process.env.NODE_ENV !== "production") {
          console.warn("[webxpay] prepare-redirect failed", {
            cart_id: cartId,
      ...
      if (process.env.NODE_ENV !== "production") {
        console.info("[webxpay] prepare-redirect ok", {
          cart_id: cartId,
          attempt_id: prepared.data.attempt_id,
          action_host: safeHost(prepared.data.action_url),
          amount: prepared.data.amount,
          currency_code: prepared.data.currency_code,
        })
      }
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.info("[webxpay] auto-submitting gateway form", {
        action_host: safeHost(actionUrl),
      })
    }
  ...
  if (!fields || !actionUrl) {
    return (
      <div className="w-full max-w-md rounded-md border border-gray-100 bg-white p-6 text-center shadow-sm">
        <h1 className="text-[22px] font-bold text-[#111111]">Preparing WEBXPAY</h1>
  ...
  return (
    <div className="w-full max-w-md rounded-md border border-gray-100 bg-white p-6 shadow-sm">
      <h1 className="text-[22px] font-bold text-[#111111]">Continuing to WEBXPAY</h1>
      <p className="mt-3 text-[14px] text-[#626978]">
        Your browser will post you to WEBXPAY to pay
        {amountLabel ? ` ${amountLabel}` : ""}. If nothing happens, use the button below.
      </p>
      <form
        ref={formRef}
        method="POST"
